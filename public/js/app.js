define([
  'jquery', 'backbone', 'toastr', 'routers/product'
], 
function($, Backbone, toastr, ProductsRouter) {
  ...
  $(document).on('click', 'a:not([data-bypass])', function (evt) {
    ...
    if (href.length && href.substr(0, 1) != '#') {
      ...
      ProductsRouter.navigate(getRelativeUrl(href, rootUrl), {trigger: true});
    }
  });
  ...
  $(document).on('submit', 'form:not([data-bypass])', function (evt) {
    ...
    if (href.length && href.substr(0, 1) != '#') {
      ...
      .done(function(data) {
        ...
        ProductsRouter.navigate(getRelativeUrl(xhr.responseURL, rootUrl));
      });
    }
